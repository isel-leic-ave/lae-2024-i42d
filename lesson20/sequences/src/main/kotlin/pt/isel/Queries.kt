...
public fun <T: Any> Sequence<T>.lazyDistinct(): Sequence<T> {
    return object : Sequence<T> {
        override fun iterator(): Iterator<T> {
            ...
            fun containsElement(it: T) : Boolean {
                if(!itemsAlreadyReturned.contains(it)) {
                    itemsAlreadyReturned.add(it)
                    return true
                }
                return false
            }
            return createIterator(this@lazyDistinct.iterator(), ::containsElement)
        }
    }
}
